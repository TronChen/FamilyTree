...
class SelectMemberViewModel(
    ...
    private val _familyMember = MutableLiveData<List<User>>()
    // The external LiveData for the SelectedProperty
    val familyMember: LiveData<List<User>>
        get() = _familyMember
    ...
    fun getAllFamily(){
        coroutineScope.launch {
            ...
            when (val result = repository.getAllFamily()) {
                is AppResult.Success -> {
                    _error.value = null
                    _status.value = LoadApiStatus.DONE
                    _familyMember.value = result.data
